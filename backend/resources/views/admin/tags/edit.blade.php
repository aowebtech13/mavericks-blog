...
<div class="max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <h1 class="text-2xl lg:text-3xl font-bold text-white">Edit Tag</h1>
    ...
    <div class="rounded-xl border border-white/10 bg-night-800/60 overflow-hidden">
        <form action="{{ route('admin.tags.update', $tag) }}" method="POST" class="p-4 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $tag->name) }}" required placeholder="e.g., PHP" class="w-full rounded-lg border border-white/10 bg-night-950 px-4 py-2.5 text-base text-white transition focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
                </div>

                <div>
                    <label for="slug" class="mb-1.5 block text-sm font-semibold text-slate-400">Slug (optional)</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $tag->slug) }}" placeholder="e.g., php" class="w-full rounded-lg border border-white/10 bg-night-950 px-4 py-2.5 text-base text-white transition focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
                </div>
            </div>
